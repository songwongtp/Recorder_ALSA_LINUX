...
std::thread waveRecorder::wavThread;
bool waveRecorder::recordFlag = false;
...
void waveRecorder::onRecord() {
    recordFlag = true;
    wavThread = std::thread(&waveRecorder::recordWAV, this);
}

void waveRecorder::offRecord() {
    recordFlag = false;
    wavThread.join();
}
...
int waveRecorder::recordWAV(){
    ...
    strftime(name_buf, sizeof(name_buf), "%Y%m%d-%H%M%S_", timeinfo);
    std::string fileName, name(name_buf), type = ".wav";
    int part = 0;    
    ...
    while(recordFlag){
        if(duration == 10){
            ...
            //Reset duration and Open new file
            duration = 0;
    ...
    printf("Finished writing to %d files\n", part + 1);
    return 0;
}
